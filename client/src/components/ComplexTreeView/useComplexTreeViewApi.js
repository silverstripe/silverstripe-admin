 ...
 *
 * @param {string} treeId - The ID of the tree instance
 * @param {number} currentRecordIDParam - The ID of the currently selected record
 * @param {string} apiEndpoint - Required. The API endpoint for fetching tree data
 * @param {string|null} listUrlPattern - Optional. URL pattern for list view links. If null,
 *   listview functionality will be disabled for limited indicator nodes.
 * @param {string|null} saveNodeEndpoint - Optional. Endpoint for saving node changes
 * @param {string|null} updateNodesEndpoint - Optional. Endpoint for updating nodes
 * @throws {Error} If apiEndpoint is not provided
 */
const useComplexTreeViewApi = (
  treeId,
  currentRecordIDParam,
  apiEndpoint,
  listUrlPattern = null,
  ...
  if (!apiEndpoint) {
    throw new Error('useComplexTreeViewApi: apiEndpoint is required');
  }
  ...
  const transformDataForTree = (apiData, isSubtree = false) => {
    ...
    const processNode = (node) => {
      ...
      if (isLimited) {
        ...
        // linkUrl is only set when listUrlPattern is provided; otherwise listview is disabled
        const linkUrl = listUrlPattern
          ? listUrlPattern.replace('%s', node.id)
          : null;
        items[syntheticId] = {
          index: syntheticId,
          isFolder: false,
  ...
  const fetchData = async (rootID = 0) => {
    try {
      ...
      // The API returns currentRecordID to allow the backend to inform the frontend
      // of which record is currently active (useful when navigating from external sources)
  ...
  const saveNode = async (nodeId, parentId, siblingIds) => {
    ...
    let endpoint = saveNodeEndpoint;
    if (!endpoint) {
      const baseEndpoint = apiEndpoint.replace(/\/jsonview.*/, '');
      endpoint = `${baseEndpoint}/savejsonnode`;
    }
    const response = await fetch(endpoint, {
      method: 'POST',
      headers: {
   ...
   * Updates the tree state with the new node data without triggering a full reload.
   */
  const updateNodes = async (nodeIds) => {
    if (!data || !dataProviderRef.current) {
      return null;
    }
    let endpoint = updateNodesEndpoint;
    if (!endpoint) {
      const baseEndpoint = apiEndpoint.replace(/\/jsonview.*/, '');
      endpoint = `${baseEndpoint}/updatejsonnodes`;
    }
    const idsParam = nodeIds.join(',');
    const fullEndpoint = `${endpoint}?ids=${encodeURIComponent(idsParam)}`;
    const response = await fetch(fullEndpoint);
    if (!response.ok) {
      throw new Error(`HTTP error! status: ${response.status}`);
    }
    const result = await response.json();
    // Update the tree state with the new node data
    // This updates nodes in place without causing a full tree reload
    const updatedItems = { ...data.items };
    Object.keys(result).forEach(nodeId => {
      const nodeData = result[nodeId];
      const nodeKey = nodeId.toString();
      if (nodeData && updatedItems[nodeKey]) {
        // Update existing node's data while preserving tree structure
        updatedItems[nodeKey] = {
          ...updatedItems[nodeKey],
          data: {
            ...updatedItems[nodeKey].data,
            ...nodeData,
