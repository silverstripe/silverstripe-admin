...
test('TreeContextMenu renders with correct items based on node permissions', () => {
  ...
  expect(screen.getByText('Add page under this page')).not.toBeNull();
  expect(screen.getByText('Duplicate')).not.toBeNull();
});

test('TreeContextMenu shows "Add page under this page" submenu with allowedChildren', () => {
  const onClose = jest.fn();
  const onMenuAction = jest.fn();
  const contextMenuUrls = {
    duplicate: '/admin/pages/tree/duplicate/%s',
  ...
  const addPageItem = screen.getByText('Add page under this page').closest('.tree-context-menu__item--submenu');
...
test('TreeContextMenu shows "Duplicate" submenu with two options', () => {
  ...
  expect(screen.getByText('This page only')).not.toBeNull();
  expect(screen.getByText('This page and subpages')).not.toBeNull();
});
...
test('TreeContextMenu hides menu items for non-editable nodes', () => {
  ...
  expect(screen.queryByText('Add page under this page')).toBeNull();
...
test('TreeContextMenu calls onMenuAction for "Duplicate" actions', () => {
  ...
  const thisPageOnlyButton = screen.getByText('This page only');
  ...
  const thisPageAndSubpagesButton = screen.getByText('This page and subpages');
...
test('TreeContextMenu hides duplicate menu items when contextMenuUrls is null', () => {
  ...
  expect(screen.queryByText('This page and subpages')).toBeNull();
});
