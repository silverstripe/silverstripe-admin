...
jest.mock('../TreeContextMenu', () => ({
  __esModule: true,
  default: ({ isOpen, position, node, onClose, onMenuAction, contextMenuUrls }) => {
...
test('ComplexTreeView limited indicator child has link to show as list', async () => {
  setupMockWithData();
  render(<ComplexTreeView />);
  await waitFor(() => {
    expect(screen.getByText('More Pages')).not.toBeNull();
  ...
  fireEvent.click(expandButton);
  await waitFor(() => {
    const showAsListLink = screen.getByText('show as list');
    expect(showAsListLink).not.toBeNull();
    expect(showAsListLink.href).toContain('/admin/pages?ParentID=6');
  });
});
